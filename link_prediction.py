...
def aggregate_edge_emb(emb1, emb2, aggregation_method='Concatenation'):
    if aggregation_method == "Concatenation":
        edge_emb = np.zeros(shape=len(emb1) * 2)
        edge_emb[:len(emb1)] = emb1
        edge_emb[len(emb1):] = emb2
    elif aggregation_method == "Average":
        edge_emb = (emb1 + emb2) / 2
    elif aggregation_method == "Hadamard": # Aij * Bij = Cij
        edge_emb = emb1 * emb2

    return edge_emb

def link_prediction(train_edges, test_edges, train_edges_uncon, test_edges_uncon, embeddings, result_filename, num_embed):
    ...
    for edge in train_edges + train_edges_uncon:
        ...
        edge_emb = aggregate_edge_emb(emb1, emb2, 'Concatenation')
    ...
    with open(result_filename, "w+") as f:
        f.writelines("auc\t" + str(lp_auc_score_macro) + "\n")
