...
def parameter_parser():
    ...
    parser.add_argument("--input",
                        nargs="?",
                        default="./input/WikiElec.txt",
                        help="Training edge list.")
    parser.add_argument("--output",
                        nargs="?",
                        default="./input/WikiElec.txt",
                        help="Embedding file.")
    parser.add_argument('--directed', action='store_true', help=' ')
...
def read_edge_list(args):
    ...
    """
    G = nx.DiGraph()
    edges = np.loadtxt(args.input) # yc_edit: edge_path -> input
    for i in range(edges.shape[0]):
        G.add_edge(int(edges[i][0]), int(edges[i][1]), weight=edges[i][2])
    if args.directed == False: # yc_add
        G = G.to_undirected() 
