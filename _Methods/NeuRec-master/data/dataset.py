...
class Dataset(object):
    ...
    def _get_data_path(self, config):
        ...
        test_data_path = config['data.test.input.path']
        ...
        return data_path, test_data_path, saved_prefix
    ...
    def _check_saved_data(self, splitter, ori_prefix, saved_prefix):
        ...
        if splitter in ("loo", "ratio"):
            ...
            # yc: add
            train_file = ori_prefix + ".edgelist"
            test_file = ori_prefix + "_test.edgelist"
        ...
        for postfix in [".edgelist", "_test.edgelist", ".user2id", ".item2id"]: # yc_add
    ...
    def _load_data(self, config):
        ...
        ori_prefix, test_path, saved_prefix = self._get_data_path(config)
        ...
        # if config["data.index.remap"] == True:
        #     train_file = saved_prefix + ".edgelist"
        #     test_file = saved_prefix + "_test.edgelist"
        ...
        if self.num_users > self.num_items:
        ...
        if file_format == "UI":
            ...
            train_ratings_t = [1.0] * len(train_data["item"]) # yc_add
        else:
        ...
        self.test_matrix = csr_matrix((test_ratings, (test_data["user"], test_data["item"])),
                                      shape=(self.num_users, self.num_items))
        # yc_add
        self.train_matrix_t = csr_matrix((train_ratings_t, (train_data["item"], train_data["user"])),
