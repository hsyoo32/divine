...
class WRMF(AbstractRecommender):
    def __init__(self, sess, dataset, conf):
        ...
        cur_max = 0
        cur_min = 0
        # set origin_max_weight for min-max norm.
        if conf["weight"] == "user_oriented":
            for u in np.arange(self.num_users):
                chk_num = len(self.dataset.train_matrix[u].indices)
                if cur_max < chk_num:
                    cur_max = chk_num
        elif conf["weight"] == "item_oriented":
            for i in np.arange(self.num_items):
                chk_num = len(self.dataset.train_matrix_t[i].indices)
                if cur_max < chk_num:
                    cur_max = chk_num
        # set weights and ratings for all the node pairs
        if conf["weight"] == "user_oriented" or conf["weight"] == "uniform":
            for u in tqdm.tqdm(np.arange(self.num_users)):
                items_by_user_id = self.dataset.train_matrix[u].indices
                weight = 0

                if conf["weight"] == "user_oriented":
                    weight = len(items_by_user_id) * self.alpha / cur_max
                for i in np.arange(self.num_items):
                    if not i in items_by_user_id:
                        self.Cui[u, i] = weight
                        self.Pui[u, i] = 0.001
                    else:
                        self.Cui[u, i] = self.alpha
            ...
            for i in tqdm.tqdm(np.arange(self.num_items)):
                users_by_item_id = self.dataset.train_matrix_t[i].indices
                weight = len(users_by_item_id) * self.alpha / cur_max
                for u in np.arange(self.num_users):
                    if not u in users_by_item_id:
                        self.Cui[u, i] = weight
                        self.Pui[u, i] = 0.001
                    else:
                        self.Cui[u, i] = self.alpha
